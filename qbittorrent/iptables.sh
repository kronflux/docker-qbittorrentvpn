...
if [[ $iptable_mangle_exit_code == 0 ]]; then
	...
	# register a routing table for WebUI traffic so packets marked with fwmark 1
	# (set by the mangle rules below) return via the docker interface instead of
	# the VPN tunnel. /etc/iproute2 is not shipped by iproute2 on Debian trixie,
	# and using `>` rather than `>>` keeps this idempotent across restarts.
	mkdir -p /etc/iproute2
	echo "200	webui" > /etc/iproute2/rt_tables
